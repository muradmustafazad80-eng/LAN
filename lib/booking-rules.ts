    ...
    db
      ...
      .where(
        and(
          ...
          eq(businessHours.weekday, zoned.weekday),
        ),
      )
      .limit(1),
    ...
    db
      ...
      .where(
        and(
          ...
          eq(barberSchedules.weekday, zoned.weekday),
        ),
      )
      .limit(1),
  ])
...
export async function validateBookingWindow(input: {
  businessId: string
  barberId: string
  ...
  if (
    !schedule.business ||
    !schedule.business.isOpen ||
    !schedule.barber ||
    !schedule.barber.isWorking
  ) {
    return {
      ok: false as const,
      reason: 'Bu gün üçün xidmət mümkün deyil.',
    }
  }

  const startParts = getZonedDateAndTime(
    input.start,
    schedule.timezone,
  )

  const endParts = getZonedDateAndTime(
    end,
    schedule.timezone,
  )
  ...
  if (startParts.date !== endParts.date) {
    return {
      ok: false as const,
      reason: 'Xidmət seçilmiş iş saatlarına sığmır.',
    }
  }
  ...
  const businessStart = minutesFromTime(
    schedule.business.startTime,
  )

  const businessEnd = minutesFromTime(
    schedule.business.endTime,
  )

  const barberStart = minutesFromTime(
    schedule.barber.startTime,
  )

  const barberEnd = minutesFromTime(
    schedule.barber.endTime,
  )

  if (
    startMinutes < businessStart ||
    endMinutes > businessEnd ||
    startMinutes < barberStart ||
    endMinutes > barberEnd
  ) {
    return {
      ok: false as const,
      reason: 'Seçilmiş vaxt iş saatlarına uyğun deyil.',
    }
  }

  const blocked = await db
    .select({
      id: barberBlockedPeriods.id,
    })
    .from(barberBlockedPeriods)
    .where(
      and(
        eq(
          barberBlockedPeriods.businessId,
          input.businessId,
        ),
        eq(
          barberBlockedPeriods.barberId,
          input.barberId,
        ),
  ...
  if (blocked[0]) {
    return {
      ok: false as const,
      reason: 'Seçilmiş vaxt bu usta üçün bağlıdır.',
    }
  }
...
export async function getAvailableSlots(
  ...
  const dayStart = localDateTimeToDate(
  ...
  const zonedDay = getZonedDateAndTime(
    dayStart,
    business.timezone,
  )
  ...
  const startMinutes = Math.max(
    minutesFromTime(scheduleBusiness.startTime),
    minutesFromTime(barber.startTime),
  )

  const endMinutes = Math.min(
    minutesFromTime(scheduleBusiness.endTime),
    minutesFromTime(barber.endTime),
  )
  ...
  const blocked = await db
    .select({
      startsAt: barberBlockedPeriods.startsAt,
      endsAt: barberBlockedPeriods.endsAt,
    })
    .from(barberBlockedPeriods)
    .where(
      and(
        ...
        lt(
          barberBlockedPeriods.startsAt,
          windowEnd,
        ),
        gt(
          barberBlockedPeriods.endsAt,
          windowStart,
        ),
      ),
    )
  ...
  for (
    ...
    minute + serviceDuration <= endMinutes;
    minute += safeInterval
  ) {
    ...
    const candidate = localDateTimeToDate(
      `${zonedDay.date}T${hh}:${mm}:00`,
      business.timezone,
    )
    ...
    const conflict =
      existing.some(
        (item) =>
          item.dateTime < end &&
          item.endDateTime > candidate,
      ) ||
      blocked.some(
        (item) =>
          item.startsAt < end &&
          item.endsAt > candidate,
      )
