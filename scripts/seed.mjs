...
async function main() {
  ...
  try {
    ...
    if (businessRes.rows.length === 0) {
      throw new Error(
        'Default business not found. Run npm run db:migrate first.'
      )
    }
    ...
    for (const b of barbers) {
      ...
      await client.query(
        `INSERT INTO "Barber"
         ("id", "businessId", "userId", "name", "specialty", "experience", "image", "status")
         VALUES ($1,$2,$3,$4,$5,$6,$7,'active')
         ON CONFLICT ("businessId","name")
         DO UPDATE SET
    ...
    await client.query(
      `
      INSERT INTO "BarberSchedule"
      ("barberId","weekday","startTime","endTime","isWorking")
      SELECT
        "id",
        d.weekday,
        CASE
          WHEN d.weekday = 0 THEN '11:00'::time
          ELSE '10:00'::time
        END,
        CASE
          WHEN d.weekday = 0 THEN '20:00'::time
          ELSE '22:00'::time
        END,
        true
      FROM "Barber"
    ...
    const demoCustomer = await client.query(
      `INSERT INTO "User"
       ("id", "businessId","email","passwordHash","role")
       VALUES ($1,$2,'customer@kralbarber.local',$3,'customer')
       ON CONFLICT ("businessId","email")
       DO UPDATE SET
         "passwordHash"=EXCLUDED."passwordHash",
         "role"='customer'
    ...
    console.log(
      'Barber: elvin@kralbarber.local / resad@kralbarber.local / kamran@kralbarber.local'
    )
  } catch (error) {
